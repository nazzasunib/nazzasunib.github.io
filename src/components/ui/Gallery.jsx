...
export function Shot({ shot, className = '', sizes, eager = false, fit }) {
  ...
  return (
    <div className={`shot ${className}`} data-fit={fit}>
...
function Filmstrip({ photos, smooth, fit }) {
  const n = photos.length;
...
/* ---------------------------------------------------------------------------
   StoryModal — filmstrip plus article, shared by Events and Achievements.

   `entry` supplies `photos`, `meta` and `article: { title, blocks }`. `fit`
   decides how a frame fills the viewport: 'cover' crops to the window,
   'contain' letterboxes it, which is what portrait certificates need.
--------------------------------------------------------------------------- */
export function StoryModal({ entry, onClose, fit = 'cover', tall = false }) {
  ...
  useEffect(() => {
    /* a single photo has nowhere to advance to */
  ...
  return (
    ...
    >
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={entry.article.title}
      ...
      >
        ...
        <div
          className={`slideshow ${tall ? 'slideshow-tall' : ''}`}
          ref={viewport}
        ...
        >
          <Filmstrip photos={photos} smooth={smooth} fit={fit} />
        ...
        {n > 1 && (
          <div className="dots" aria-label="Choose photo">
            {photos.map((p, k) => (
              <button
                key={p.src}
        ...
        <article className="story" data-lenis-prevent>
          <span className="eyebrow">{entry.meta}</span>
          <h2>{entry.article.title}</h2>
          {entry.article.blocks.map((b, k) => (
            <Block block={b} key={k} />
          ))}
        </article>
      </motion.div>
